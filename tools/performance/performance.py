...
def get_dims(log_files):
    ...
    for log_file in log_files:
        dims.append(int((log_file.split('.')[0]).split('_')[-4]))
...
def read_data(methods, dims, data_path, log_files):
    ...
    for log_file in log_files:
        dim = int((log_file.split('.')[0]).split('_')[-4])
...
def draw_line_chart(methods, dims, data, figure_name, y_step, y_label, title):
...
def analyze_data(data_path):
    ...
    draw_line_chart(methods, dims, data_throughput, data_path + 'throughput.png',
                    1, 'Throughput / TFLOPS', 'BACK2BACK HGEMM Throughput')
    draw_line_chart(methods, dims, data_performance,  data_path + 'performance.png', 10,
                    'Performance Compared with Cublas / %', 'BACK2BACK HGEMM Performance')
