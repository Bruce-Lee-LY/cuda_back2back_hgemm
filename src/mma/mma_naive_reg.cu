...
//
// Description: mma naive reg back2back hgemm b16x8_w16x8_p16x16_w16x16
...
__global__ void mmaNaiveRegKernel(const half *__restrict__ A, const half *__restrict__ B, const half *__restrict__ C,
...
#pragma unroll
    ...
    HMMA16816(RD[0], RD[1], RP[0][0][0], RP[0][0][1], RP[0][1][0], RP[0][1][1], RC[0], RC[1], RD[0], RD[1]);
...
void mmaNaiveReg(half *A, half *B, half *C, half *D, size_t M, size_t L, size_t N, size_t K) {
    dim3 block(WARP_SIZE);
    dim3 grid(div_ceil(L, GEMM1_WARP_COLS), div_ceil(M, GEMM1_WARP_ROWS));

    mmaNaiveRegKernel<<<grid, block>>>(A, B, C, D, M, L, N, K);
}
