...
//
// Description: mma async reg back2back hgemm b128x8_w16x8_p128x16_w16x16_k128
...
#define C_SMEM_STRIDE 24  // GEMM0_BLOCK_COLS + SMEM_PADDING

#define D_SMEM_STRIDE 16  // GEMM1_BLOCK_COLS + SMEM_PADDING
#define D_SMEM_OFFSET 8   // GEMM1_WARP_COLS

__global__ void mmaAsyncRegKernel(const half *__restrict__ A, const half *__restrict__ B, const half *__restrict__ C,
    ...
    __shared__ half C_smem[GEMM1_BLOCK_COLS][C_SMEM_STRIDE];
    ...
    uint32_t RP[GEMM0_WARP_COL_TILES][GEMM0_WARP_ROW_TILES][2];
...
#pragma unroll
                for (size_t j = 0; j < GEMM0_WARP_ROW_TILES; ++j) {
                    HMMA16816(RP[i][j][0], RP[i][j][1], RA[i][0], RA[i][1], RA[i][2], RA[i][3], RB[j][0], RB[j][1],
                              RP[i][j][0], RP[i][j][1]);
                }
            }
        }
...
#pragma unroll
    ...
    if (warp_id == 0 && lane_id < MMA_N * 2) {
        ...
        uint32_t C_smem_lane_addr =
            __cvta_generic_to_shared(&C_smem[lane_id / 2][0]) + (lane_id % 2) * THREAD_COPY_BYTES;
...
#pragma unroll
    for (size_t j = 0; j < GEMM1_WARP_ROW_TILES; ++j) {
        size_t C_smem_idx = (warp_id % GEMM1_BLOCK_ROW_WARPS) * GEMM1_WARP_COLS + j * MMA_N;
        uint32_t C_smem_lane_addr =
            __cvta_generic_to_shared(&C_smem[C_smem_idx + lane_id % 8][((lane_id / 8) % 2) * 8]);
...
#pragma unroll
        for (size_t j = 0; j < GEMM1_WARP_ROW_TILES; ++j) {
            HMMA16816(RD[i][j][0], RD[i][j][1], RP[i][2 * j][0], RP[i][2 * j][1], RP[i][2 * j + 1][0],
                      RP[i][2 * j + 1][1], RC[j][0], RC[j][1], RD[i][j][0], RD[i][j][1]);
        }
    }
...
size_t initMmaAsyncReg() {
    ...
    size_t smem_max_size = dynamic_smem_size + GEMM1_BLOCK_COLS * C_SMEM_STRIDE * sizeof(half);
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(mmaAsyncRegKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void mmaAsyncReg(half *A, half *B, half *C, half *D, size_t M, size_t L, size_t N, size_t K) {
    static size_t smem_max_size = initMmaAsyncReg();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(div_ceil(L, GEMM1_BLOCK_COLS), div_ceil(M, GEMM1_BLOCK_ROWS));

    mmaAsyncRegKernel<<<grid, block, smem_max_size>>>(A, B, C, D, M, L, N, K);
}
